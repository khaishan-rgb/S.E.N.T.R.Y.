...
def rank_candidates(cands, W=None):
    """Operational priority order for a halfway/OS insertion.

    1) best resulting on-road headway (smallest maximum gap),
    2) best EWT proxy from the resulting local gaps,
    3) earliest time to reach / insert,
    4) least service skipped, then shortest off-service distance.

    This deliberately avoids picking an early-route stop merely because it is geographically close.
    """
    for c in cands:
        hw = c.get("hw") or {}
        gaps = [float(x) for x in (hw.get("gaps_half") or [])]
        H = max(0.1, float(c.get("target_hw") or 0.0))
        # EWT proxy: excess waiting generated by uneven local headways.  It is a ranking
        # proxy only; the Recovery Optimiser remains the authoritative multi-point EWT engine.
        ewt = sum((g-H)**2 for g in gaps) / (2.0 * H * max(1, len(gaps))) if H and gaps else 999.0
        c["ewt_proxy"] = round(ewt, 3)
        c["result_max_hw"] = round(float(hw.get("max_half", 999.0)), 2)
        c["skip_stops"] = int(c.get("stops_omitted", 0))
    cands.sort(key=lambda c: (
        c["result_max_hw"], c["ewt_proxy"], float(c.get("off_min", 999)),
        c["skip_stops"], int(c.get("important_missed", 0)), float(c.get("off_km", 999))
    ))
    for i, c in enumerate(cands, 1):
        c["rank"] = i
        c["score"] = round(1000 - c["result_max_hw"]*20 - c["ewt_proxy"]*10 - float(c.get("off_min",0)) - c["skip_stops"]*.5, 2)
    return cands
